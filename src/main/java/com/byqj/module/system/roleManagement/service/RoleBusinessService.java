...
public class RoleBusinessService {
    ...
    public void addRoleRequestProcess() {
        ...
        logCenterService.setLogTypeIsSystem();
    ...
    public void deleteRoleRequestProcess() {
        ...
        // 记录日志
        String operationContent = String.format(LogConstant.DELETE_AUTHORITY_GROUP_TEMPLATE, roleCodeList);
        logCenterService.setLogTypeIsSystem();
        logCenterService.setContent(operationContent);
        logCenterService.setResultIsTrue();
        ResponseDataUtil.setHeadOfResponseDataWithSuccessInfo(responseData);
    ...
    public void modifyRoleRequestProcess() {
        ...
        // 获取主键插入到role_acl表中
        SysRoleAcl sysRoleAcl = new SysRoleAcl();
        String acl = StringUtils.join(permissions, AclSplitConstants.REGEX);
    ...
    public void getRoleAclRequestProcess() {
        String roleCode = dataCenterService.getData("roleCode");
        //获取所有部门及其权限
        List<AclModuleLevelDto> getAllModuleList = sysAclModuleDao.getModuleWithAcl(TypeConstants.BACKGROUND, StatusConstant.ENABLE);
        //获取当前登录用户的所有权限
        ...
        //判断 checked 与 hasAcl 状态
        for (AclModuleLevelDto aclModuleLevelDto : getAllModuleList) {
        ...
        //权限模块dto转tree
        List<AclModuleLevelDto> aclModuleLevelTreeList = aclModuleListToTree(getAllModuleList);

        ResponseData responseData = dataCenterService.getResponseDataFromDataLocal();
        ResponseDataUtil.setHeadOfResponseDataWithSuccessInfo(responseData);
        ResponseDataUtil.putValueToData(responseData, "roleAcl", aclModuleLevelTreeList);
    }
